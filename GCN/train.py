...
def main():
    ...
    trainDS = NetlistGraphDataset(root=osp.join(ROOT_DIR),filePath=datasetDict[datasetChoice][0])
    testDS = NetlistGraphDataset(root=osp.join(ROOT_DIR),filePath=datasetDict[datasetChoice][1])
    ...
    def replaceLambda(data):
        return addNormalizedTargets(data,targetStats,meanVarTargetDict,targetVar=targetLbl)

    trainDS.transform = transforms.Compose([replaceLambda])
    testDS.transform = transforms.Compose([replaceLambda])
